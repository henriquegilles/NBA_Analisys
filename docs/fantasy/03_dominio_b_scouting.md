...
- [x] Definir o universo de escolas × temporadas a raspar — constantes no topo de `college.py`. **Escalado 2026-06-21 para 18 escolas × 15 temporadas (2010-11 a 2024-25)**: núcleo (duke, kentucky, kansas, north-carolina, ucla, gonzaga) + arizona, villanova, michigan-state, texas, florida, auburn, southern-california, tennessee, alabama, baylor, arkansas, connecticut. 3733 player-seasons. Escalar mais editando `SCHOOLS`/`SEASONS`.
...
- [x] **`int_prospect__nba_bridge` + `fct_college_to_nba_outcomes`** — feito 2026-06-19 (test PASS=3). **Atalho importante (D-29):** o desfecho NBA veio do seed `draft` que já existia (médias de carreira pg_pts/trb/ast + WS/BPM/VORP) — não precisou de scraper novo. Ponte por nome + janela de ano do draft. 96 outcomes históricos no slice inicial (→ 279 após escalar o backbone em 2026-06-21); xará resolvido via override (D-09).
- [x] **`fct_prospect_scouting`** — feito 2026-06-19 (test PASS=4): projeção = média dos desfechos dos comps que chegaram à NBA. 243 prospectos no slice inicial → **742 após escalar o backbone** (2026-06-21). Ex.: Cooper Flagg (2024-25) → 15.8 pts / 6.5 reb projetados de 4 comps NBA.
- [x] **Carreira NBA 6-cat completa (D-30)** — feito 2026-06-21. `src/scraping/nba_careers.py` raspa a linha de total ("Career"/"N Yrs") da página do jogador (stocks/3PM/TOV) e junta pelo slug NBA `bbr_id` (capturado no `draft.py`, agora por `data-stat` — runbook #25). Scrape ao vivo: 279 carreiras (pós-escala); as 6 cats fluem (Outcomes 279, Scouting 742).
