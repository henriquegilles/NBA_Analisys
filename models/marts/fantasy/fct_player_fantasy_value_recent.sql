...
-- Grão: 1 linha por jogador. Valor fantasy na FORMA RECENTE (últimos 15 jogos).
--
-- Mesma lógica de z-score do fct_player_fantasy_value_season, mas a janela é
-- só os últimos N=15 jogos de cada jogador (D-16). O pool de referência é
-- RECOMPUTADO sobre essa janela (D-19) — médias e desvios saem das médias dos
-- últimos 15 jogos, não da temporada cheia.
--
-- Floor do pool: pelo menos 10 dos últimos 15 jogos jogados E >= 12 min/jogo
-- (a janela é curta, então exigimos presença real para a forma ser confiável).
-- Pesos iguais (D-20); expõe soma E média (D-22).
...
-- Ordena os jogos de cada jogador do mais recente para o mais antigo
ranked as (
    select
        *,
...
-- Mantém só os últimos N jogos por jogador
recent_games as (
    select * from ranked
    where game_recency <= {{ window_n }}
),

-- Médias por jogo de cada jogador NA JANELA recente
per_player as (
    select
        player_key,
        bbr_id,
...
-- Pool de referência recomputado sobre as médias da janela recente
pool_stats as (
    select
