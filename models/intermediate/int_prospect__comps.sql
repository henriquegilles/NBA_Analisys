-- int_prospect__comps
-- Grão: prospecto × comp (k linhas por prospecto).
--
-- A MECÂNICA de comparáveis do scouting (Domínio B), prototipada SEM o lado NBA:
-- para cada prospecto, acha os k jogadores college mais parecidos. Quando o
-- backbone college→NBA existir, `fct_prospect_scouting` junta estes comps aos
-- desfechos NBA para gerar a projeção.
--
-- Decisões aplicadas:
--   D-12  prospecto = temporada MAIS RECENTE de cada jogador.
--   D-21  distância EUCLIDIANA sobre features PADRONIZADAS (z-score), k ≈ 8–10.
--         Features (9): 6 categorias por-40 + class_rank + TS% + SOS.
--   D-23/D-28  comps no mesmo arquétipo (G/F/C), com FALLBACK para todos os
--         arquétipos quando o mesmo-arquétipo tem menos de k candidatos.
--
-- Nulls de feature são neutralizados (coalesce para a média → contribuição 0 na
-- distância), para não quebrar nem dominar o cálculo.

-- Materializado como TABELA (exceção ao view-padrão do intermediate): o k-NN
-- faz cross-join de 3.7k player-seasons e como view recomputava TUDO a cada
-- consulta do painel (aba Comps levava ~8 min por query — Rodada 6 Fase 2).
{{ config(materialized='table') }}

{% set k = 8 %}

{% set features = [
    'pts_per_40', 'trb_per_40', 'ast_per_40', 'stocks_per_40',
    'three_p_per_40', 'tov_per_40', 'class_rank', 'ts_pct', 'team_sos'
] %}

with prospects as (
    -- Temporada mais recente de cada jogador (D-12).
    select *
    from (
        select *,
               row_number() over (partition by cbb_id order by season desc) as rn_recent
        from {{ ref('int_prospect__college_stats') }}
    ) s
    where rn_recent = 1
),

-- Média e desvio-padrão de cada feature sobre o pool de prospectos.
stats as (
    select
        {% for f in features %}
        avg({{ f }})              as {{ f }}_mean,
        stddev_samp({{ f }})      as {{ f }}_std{{ "," if not loop.last }}
        {% endfor %}
    from prospects
),

-- Features padronizadas (z-score). Null → média → z = 0 (neutro).
z as (
    select
        p.cbb_id,
        p.player_name,
        p.season,
        p.archetype,
        {% for f in features %}
        (coalesce(p.{{ f }}, s.{{ f }}_mean) - s.{{ f }}_mean) / nullif(s.{{ f }}_std, 0) as z_{{ f }}{{ "," if not loop.last }}
        {% endfor %}
    from prospects p
    cross join stats s
),

-- Todos os pares prospecto × candidato (exclui o próprio jogador).
pairs as (
    select
        a.cbb_id      as prospect_id,
        a.player_name as prospect_name,
        a.season      as prospect_season,
        a.archetype   as prospect_archetype,
        b.cbb_id      as comp_id,
        b.player_name as comp_name,
        b.season      as comp_season,
        b.archetype   as comp_archetype,
        (a.archetype = b.archetype) as same_archetype,
        sqrt(
            {% for f in features %}
            power(a.z_{{ f }} - b.z_{{ f }}, 2){{ " +" if not loop.last }}
            {% endfor %}
        ) as distance
    from z a
    join z b on a.cbb_id <> b.cbb_id
),

-- Quantos candidatos do mesmo arquétipo cada prospecto tem?
arch_counts as (
    select prospect_id, count(*) filter (where same_archetype) as n_same_archetype
    from pairs
    group by prospect_id
),

-- Se há >= k do mesmo arquétipo, restringe a ele; senão, libera todos (fallback).
scoped as (
    select
        p.*,
        ac.n_same_archetype,
        (ac.n_same_archetype < {{ k }}) as used_archetype_fallback
    from pairs p
    join arch_counts ac using (prospect_id)
    where p.same_archetype
       or ac.n_same_archetype < {{ k }}
),

ranked as (
    select
        *,
        row_number() over (partition by prospect_id order by distance) as comp_rank
    from scoped
)

select
    prospect_id,
    prospect_name,
    prospect_season,
    prospect_archetype,
    comp_rank,
    comp_id,
    comp_name,
    comp_season,
    comp_archetype,
    round(distance::numeric, 3) as distance,
    used_archetype_fallback
from ranked
where comp_rank <= {{ k }}
